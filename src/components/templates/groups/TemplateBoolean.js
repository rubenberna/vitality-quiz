...
class TemplateBoolean extends Component {

  renderQuestions = () => {
    const { questions } = this.props.data
    if (questions) {
       return questions.map( question =>
          <div key={question.id} className='form-quiz-question boolean'>
            { question.moreisbetter === 1 ? '*MB ': ''}
            {question.text}
            <Switch offLabel="Nee" onLabel="Ja" id={`switch-${shortid.generate()}`} onClick={ e => this.props.handleBooleanChange(e, question) }/>
          </div>
      )
    }
  }
...
export default composedTemplateWrapper(TemplateBoolean);
